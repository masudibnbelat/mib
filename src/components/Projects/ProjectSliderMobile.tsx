      ...
      <div className="flex justify-center">
        ...
        >
          {projects.map((project) => (
            <SwiperSlide
              key={project._id}
              className="group cursor-pointer rounded-2xl overflow-hidden border border-(--color-active-border) bg-(--color-bg)"
              onClick={() => handleSlideClick(project)}
            >
              {/* Image */}
              <div className="relative h-40 w-full overflow-hidden bg-(--color-active-bg)">
                {project.images?.[0] ? (
                  <Image
